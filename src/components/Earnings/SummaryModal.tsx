...
const SummaryModal: React.FC<SummaryModalProps> = ({ isOpen, onClose, details }) => {
  ...
  const RedeemEarnedPoints = async () => {
      ...
      try {
          ...
          if (responseData) {
              ...
              // Wait a short time before closing to allow user to see message
              setTimeout(() => {
                onClose();
              }, 2000);
          } 
            ...
            {alertMessage && (
              <div 
                className={`mt-4 p-3 rounded text-center font-medium ${
                  alertType === 'success' ? 'bg-green-100 text-green-700 border border-green-400' : 
                  alertType === 'error' ? 'bg-red-100 text-red-700 border border-red-400' : ''
                }`}
              >
                {alertMessage}
              </div>
