...
const UserProfile = () => {
  const userData2: any = window.localStorage.getItem("userData")
  const parsedUserData = JSON.parse(userData2)
  ...
  // Get userId from location state or session storage as fallback
  ...
  useEffect(() => {
    ...
    if (stateUserId) {
      // If userId exists in state, save it to sessionStorage for persistence
      sessionStorage.setItem("currentProfileUserId", stateUserId)
      setUserId(stateUserId)
    } else {
      // If not in state, try to get from sessionStorage
      const storedUserId = sessionStorage.getItem("currentProfileUserId")

      if (storedUserId) {
        setUserId(storedUserId)
      } else {
        // If no userId is found anywhere, redirect to a fallback page or show error
        toast.error("User ID not found. Redirecting to home page.")
        navigate("/")
      }
    }
  ...
  // Only proceed with data fetching if userId is available
  useEffect(() => {
    if (userId) {
      loadDefaultData();
  ...
  const url = userId ? `${baseURL}/Campaign/UserCampaigns?AccountId=${userId}&pageNumber=${page}&pageSize=10` : ""
